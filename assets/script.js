// Utility Functions
function encodeBlogTitle(title) {
	let fixedUrl = title.replaceAll(" ", "-")
		.replaceAll(",", "-")
		.replaceAll("'", "-")
		.replaceAll("(", "-")
		.replaceAll(")", "-");
	return "/blogPost.html?title=" + fixedUrl;
}

function fixBlogTitle(title) {
	let fixed = title.replaceAll(" ", "-")
		.replaceAll(",", "-")
		.replaceAll("'", "-")
		.replaceAll("(", "-")
		.replaceAll(")", "-");
...
if (page == "projects.html") {
	newProjectCard("DeviceSyncApp", "DeviceSyncApp", "[No Longer In Active Development] DeviceSync is an application made by TheProtonDev which aims to provide the userbase with a cross platform, synchronized clipboard that can be extended to their desires if the user is knowledgeable enough", "https://theprotondev.github.io/DeviceSyncApp/index.html");
	...
	newProjectCard("MaskDetection", "MaskDetection", "This is an algorithm I made for a project in my STEM Pathway Class using TeachableMachine and a set of Python scripts I came up with to scrape data in large amounts for accuracy");
}
